export default {
   ...
   <div id="app" class="container">
        ...
        <table  style="width: 100%; border-collapse: collapse; text-align: left; margin-bottom: 20px;">
            ...
            <tbody v-if = "services.length > 0">
                <tr v-for="service in services" :key="service.id" style="border: 1px solid #ddd; background-color: #fff;" >
                    ...
                    <td style="border: 1px solid #ddd; padding: 10px;">{{ professional.service_name }}</td>
                    ...
                    <button class="close-btn" @click="book-request('service.id')"
    ...
    methods:{
        async search(){
            const res = await fetch(location.origin + `/search/${this.selectedCategory}/${this.searchQuery}`, {
                method: "GET",
                headers: {
